    ...
    describe("Segment Visitor Matcher", function() {
      ...
      var verifyResult = function(result, expected) {
        ...
        for (var i = 0; i < result.length; i++) {
          ...
          chai.assert(result[i].time.getTime() === expected[i].time.getTime(), "expected " + JSON.stringify(expected) + ", but " + JSON.stringify(result));
        }
      };

      var getDateBySec = function(seconds) {
        return moment(baseDate).add(seconds, 's').toDate();
      }
      ...
      describe("Been To All Matcher", function() {
        describe("at least one", function() {
          ...
          describe("all fulfilled", function() {
            ...
            it("Been All", function() {
              var result = matcher.doMatch(_.extend({hasBeen: true, to: 'all'}, criteria), installationIds, encounters, encounters[encounters.length-1].exitedAt);
              verifyResult(result, [{time: getDateBySec(1), delta: 1}]);
            });

            it("Not Been All", function() {
              var result = matcher.doMatch(_.extend({hasBeen: false, to: 'all'}, criteria), installationIds, encounters, encounters[encounters.length-1].exitedAt);
              verifyResult(result, [{time: getDateBySec(1), delta: -1}]);
            });

            it("Been Any", function() {
              var result = matcher.doMatch(_.extend({hasBeen: true, to: 'any'}, criteria), installationIds, encounters, encounters[encounters.length-1].exitedAt);
              verifyResult(result, [{time: getDateBySec(1), delta: 1}]);
            });

            it("Not Been Any", function() {
              var result = matcher.doMatch(_.extend({hasBeen: false, to: 'any'}, criteria), installationIds, encounters, encounters[encounters.length-1].exitedAt);
              verifyResult(result, [{time: getDateBySec(1), delta: -1}]);
            });
          });
