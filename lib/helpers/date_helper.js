...
DateHelper.setUpDatePicker = function(tmpl) {
    if (Meteor.isClient && tmpl && tmpl.data) {
        var self = tmpl;
        var format = "MMM DD, YYYY";
        $('#datepicker-container').show();
        $('#datepicker').html(
            moment(+self.data.from).format(format)
                + " to "
                + moment((+self.data.to) - 1).format(format));//stupid jqueryrangepicker has inclusive to , e.g 0 - 1000, is defined as 0 - 999
        $('#datepicker').daterangepicker({
            ...
            endDate: moment((+self.data.to) - 1)//stupid jqueryrangepicker has inclusive to , e.g 0 - 1000, is defined as 0 - 999
