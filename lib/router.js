...
LocationController = RouteController.extend({
  ...
  data: function(){
    ...
    if (!this.params.locationId) { return; }
    var loc = Locations.findOne(this.params.locationId)
    if(!loc){ return; }
    var company = Companies.findOne(loc.companyId)
    if(!company){ return; }
    var data = loc;
...
Router.map(function() {
  ...
  this.route('dashboard_segmentation', {
  ...
  this.route('dashboard_segmentation_default', {
    path:'/segmentation/',
    template:"dashboard_segmentation",
    controller: DrillingController
  });
