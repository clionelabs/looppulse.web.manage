Router.configure({
  ...
  // data: function(){
    //This part will be overrode, commented for remark
  // },
  waitOn: function() {
    ...
    if (companyId) {
      ...
      return [
                ...
                Meteor.subscribe('owned-categories', companyId)
             ] ;
    } else if (Roles.userIsInRole(Meteor.user(), ['admin'])){
      console.log("Global Base Subscription", companyId, Meteor.user())
      return [
...
LocationController = RouteController.extend({
  ...
  data: function() {
    ...
    // FIXME fix greeting page to not link with null ID
    if (!locationId || locationId === 'null') {
      // FIXME dirty way to show default location dashboard
      // this assume user have only one access to one location
      var defaultLocation = Locations.findOne({});
      if (defaultLocation) {
...
DrillingController = LocationController.extend({
  ...
  waitOn: function(){
    var companyId = this.params.companyId || LocationsHelper.getCompanyId();
    console.info("Section Subscription: ", companyId)

    return [
              Meteor.subscribe('owned-products', companyId),
              Meteor.subscribe('owned-categories', companyId),
              Meteor.subscribe("owned-segments", companyId),
              Meteor.subscribe("owned-campaigns", companyId),
              Meteor.subscribe("owned-floors", companyId)
            ]
  }
...
Router.map(function() {
  ...
  this.route('company', {
    ...
    template: "company_view",
    data: function(){ return Companies.findOne({_id: this.params.companyId}) },
    onBeforeAction: function (pause) {
      AccountsHelper.signInAsCompanyUser(this, pause, this.params.companyId)
    }
  ...
  this.route('admin_company_show', {
    ...
    yieldTemplates:{
      "admin_company_header" : { to: "view-header" },
      "admin_company_actions": { to: "row-prepend" }
    }
  });
