...
SegmentCreateController = RouteController.extend({
  ...
  onBeforeAction: function (pause) {
    ...
    if (self.data()) {
      ...
      self.subscribe('locationsFloors', locationIds);
    }
  },
  data: function () {
    var companyId = Session.get('currentCompanyId');
...
SegmentDetailController = RouteController.extend({
  ...
  waitOn: function () {
    ...
    var to = this.params.query.to ? +this.params.query.to : DateHelper.getTodayTimestamp();
  ...
  data: function () {
    ...
    var data = {};
    data.from = self.params.query.from ? +self.params.query.from : DateHelper.getSevenDaysAgoTimestamp();
    data.to = self.params.query.to ? +self.params.query.to : DateHelper.getTodayTimestamp();
    ...
    if (self.ready()) {
      ...
      var dateXNumberOfVisitorsBarChart
          = Metrics.findOne(
              _.extend(selector, {graphType: SegmentMetric.Graph.DayXNumOfVisitorLineChart}));
      var percentageOtherSegmentChart
          = Metrics.findOne(
              _.extend(selector, {graphType: SegmentMetric.Graph.VisitorOtherSegmentsBarChart}));
      var averageDwellTimeXNumberOfVisitorsChart
          = Metrics.findOne(
              _.extend(selector, {graphType: SegmentMetric.Graph.AverageDwellTimeBucketXNumOfVisitorHistogram}));
      var averageDwellTimePunchCard
          = Metrics.findOne(
              _.extend(selector, {graphType: SegmentMetric.Graph.DwellTimePunchCard}));
      var numberOfVisitsXNumberOfVisitorsBarChart
          = Metrics.findOne(
              _.extend(selector, {graphType: SegmentMetric.Graph.NumberOfVisitXNumberOfVisitorsBarChart}));
      var enteredAtPunchCard
          = Metrics.findOne(
              _.extend(selector, {graphType: SegmentMetric.Graph.EnteredAtPunchCard}));
      var exitedAtPunchCard
          = Metrics.findOne(
          _.extend(selector, {graphType: SegmentMetric.Graph.ExitedAtPunchCard}));
