...
SegmentDetailController = RouteController.extend({
  ...
  waitOn: function () {
    ...
    return [
      Meteor.subscribe('companies'),
      Meteor.subscribe('companySegments', companyId),
      Meteor.subscribe('segmentListMetrics', from , to, segmentId)
    ];
  },
  onBeforeAction: function (pause) {
    AccountsHelper.signInAsAdmin(this, pause);
  ...
  data: function () {
    ...
    if (self.ready()) {
      ...
      var segmentMetric = Metrics.findOne();
      //TODO change to new metrics
      console.log(JSON.stringify(segmentMetric));
      data.name = segment.name;
      data.list = {
        numberOfVisitors : segmentMetric.data.numberOfVisitors,
        averageDwellTime: segmentMetric.data.averageDwellTime,
        repeatedVisitorPercentage: segmentMetric.data.repeatedVisitorPercentage
      };
