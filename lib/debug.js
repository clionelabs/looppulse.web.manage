...
Debug.observeMetrics = function() {
  var printNewMetric = function(m) {
    if (m.locationId) {
      console.info("[Metric] "+m._id+" created for "+Debug.location(m.locationId));
    } else {
      console.info("[Metric] "+m._id+" created");
    }
  };
  var printChangedMetric = function(n, o) {
    console.info("[Metric] updated: " + JSON.stringify({
      _id: n._id,
      visitCount: n.visitCount,
      dwellTime: n.dwellTime,
      repeatedVisitCount: n.repeatedVisitCount
    }));
  };
  Metrics.find().observe({
    _suppress_initial: true,
    "added": function(metric) { printNewMetric(metric); },
    "changed": function(metric, oldMetric) { printChangedMetric(metric, oldMetric); }
  });
};
