...
Segment.prototype.match = function(encounter) {
  ...
  if (encounter.isHappenedInCompany(companyId)) {
    return false;
  }
  ...
  return new SegmentMatchCriteria(criteria).match(companyId, encounter);
};
...
SegmentMatchCriteria.prototype.match = function(companyId, encounter) {
  var criteria = this.criteria;
...
SegmentMatchCriteria.prototype.updateEncounterSelectorByCriteriaDays = function() {
  ...
  if (criteria.days) {
    this.encounterSelector.enteredAt = {
      $gte: MetricsHelper.nDaysAgo(this.now, criteria.days.inLast)
    };
  }
};
