...
Application.prototype.authenicatedResponse = function(token) {
  ...
  if (token != self.token) {
    ...
    var systemConfig = {};
    systemConfig["onlySendBeaconEventsWithKnownProximity"] = false;
    var firebaseConfig = {};
    firebaseConfig["root"] = "https://looppulse-dev-thomas.firebaseio.com";
    systemConfig["firebase"] = firebaseConfig;
    systemConfig["configurationJSON"] = "https://looppulse-config.firebaseio.com/companies/-JVKQSNOBjU6G3Ak_Wi7.json";
    var parseConfig = {};
    ...
    systemConfig["parse"] = parseConfig;

    response["system"] = systemConfig;
  }
  return response;
}
