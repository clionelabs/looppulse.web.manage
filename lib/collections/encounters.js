...
Encounter.findClosed = function () {
  return Encounters.find({"exitedAt": {"$exists": true}});
}
...
Encounter.createOrUpdate = function (beaconEvent) {
  ...
  if (beaconEvent.isEnter()) {
    ...
    encounter = Encounters.findOne({visitorId: beaconEvent.visitorId, installationId: installation._id, enteredAt: beaconEvent.createdAt});
    if (encounter) {
