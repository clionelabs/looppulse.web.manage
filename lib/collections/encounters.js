...
Encounters.findClosedByVisitorsInTimePeriod = function(visitors, from, to) {
  var enteredAtRange = { $gte : from };
  if (to) {
    _.extend(enteredAtRange, { $lt : to }); // end-date should be exclusive to avoid double counting encounters (which occured exactly on 00:00:00) across different periods
  }
  var selector = { visitorId : { $in : visitors }, enteredAt: enteredAtRange};
...
Encounter.prototype.save = function() {
  ...
  var modifier = {
    $set: {
      exitedAt: (this.exitedAt) ? this.exitedAt.valueOf() : null,
