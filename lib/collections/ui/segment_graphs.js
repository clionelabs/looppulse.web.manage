/**
 * A data representation of a graph,
 * @type {Meteor.Collection}
 */
SegmentGraphs = new Meteor.Collection("uiSegmentGraphs", {
    transform: function(doc) {
        ...
        if (doc.graphType === SegmentGraphBase.Graph.List) {
            ...
            return new SegmentGraphDistributionDwell(doc.segment, doc.from, doc.to, SegmentGraphBase.Graph.Data.Enter, doc.data);

        } else if (doc.graphType === SegmentGraphBase.Graph.DistributionDwellExit) {
            return new SegmentGraphDistributionDwell(doc.segment, doc.from, doc.to, SegmentGraphBase.Graph.Data.Exit, doc.data);

        } else if (doc.graphType === SegmentGraphBase.Graph.DistributionVisitsEnter) {
            return new SegmentGraphDistributionVisits(doc.segment, doc.from, doc.to, SegmentGraphBase.Graph.Data.Enter, doc.data);

        } else if (doc.graphType === SegmentGraphBase.Graph.DistributionVisitsExit) {
            return new SegmentGraphDistributionVisits(doc.segment, doc.from, doc.to, SegmentGraphBase.Graph.Data.Exit, doc.data);
